#!/usr/bin/env bash
#
# This script provisions testing environment using 'k3d'(K3s cluster in Docker)
# and execute end-to-end Capact tests.
#
# It requires Docker to be installed.
# Dependencies such as Helm v3 and k3d can be installed on demand.
...
main() {
    ...
    if [[ "${SKIP_DEPS_INSTALLATION}" == "false" ]]; then
        ...
        echo "Skipping k3d and helm installation cause SKIP_DEPS_INSTALLATION is set to true."
    fi
